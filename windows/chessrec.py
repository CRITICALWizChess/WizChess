...
# sends over a diy 4- bit parallel connection (used to, now it is a placeholder)
def parsend(sending):
	print("Sent")
		#raw_input("next\n")
...
def speechinput():
	...
	try:
		while(speak):
			press = buttons()
			if (press == "'"): # remove when button added
...
def manualinput():
	...
	senddisplay("Choose your start", "and end intersection", "'/' to scroll", "or 'enter' to confirm")
	sleep(2)
	col = row = col1 = row1 = 0
...
def moveinputconvert(firsttime, select):
	...
	while(select == 0 and firsttime == 0):
		senddisplay("press '  for voice", "or 'Enter' for manual", " ", " ")
